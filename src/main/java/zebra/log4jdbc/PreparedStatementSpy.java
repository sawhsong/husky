...
public class PreparedStatementSpy extends StatementSpy implements PreparedStatement {
	...
	public void setByte(int parameterIndex, byte x) throws SQLException {
		...
		argTraceSet(parameterIndex, "(byte)", Byte.valueOf(x));
		try {
	...
	public <T> T unwrap(Class<T> iface) throws SQLException {
		...
		try {
			...
			return (T)reportReturn(methodCall, (iface != null && (iface == PreparedStatement.class || iface == Statement.class || iface == Spy.class)) ? (T)this : realPreparedStatement.unwrap(iface));
		} catch (SQLException s) {
			reportException(methodCall, s);
			throw s;
		}
