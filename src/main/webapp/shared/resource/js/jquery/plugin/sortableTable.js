...
var Sort = (function(){
	...
	sort.date.convert = function(val) {
		// Dustin : Date convert(2016.04.22)
		var vals;
		var rgYmd = /(\d{4})[.\/-](\d{1,2})[.\/-](\d{1,2})/;
		var rgDmy = /(\d{1,2})[.\/-](\d{1,2})[.\/-](\d{4})/;

		if ($.nony.isEmpty(val)) {return 0000000000000;}
		...
		if (rgYmd.test(val)) {
			return new Date(vals[0], vals[1]-1, vals[2]).getTime();
		} else if (rgDmy.test(val)) {
			return new Date(vals[2], vals[1]-1, vals[0]).getTime();
		} else {
...
var Table = (function(){
	...
	var table = {
		...
		AutoStripeClassName:"autostripe",
		StripeClassNamePrefix:"stripeclass:",

		AutoSortClassName:"autosort",
		AutoSortColumnPrefix:"autosort:",
		AutoSortTitle:"Click to sort",
		SortedAscendingClassName:"sorted-asc",
		SortedDescendingClassName:"sorted-desc",
		SortableClassName:"sortable",
		SortableColumnPrefix:"sortable:",
		NoSortClassName:"nosort",

		AutoFilterClassName:"autofilter",
		FilteredClassName:"filtered",
		FilterableClassName:"filterable",
		FilteredRowcountPrefix:"filtered-rowcount:",
		RowcountPrefix:"rowcount:",
					...
					for (col in tdata.filters) {
						if (!hideRow) {
							...
							if (filter && col<cellsLength) {
								var val = $.trim(this.getCellValue(cells[col]));
